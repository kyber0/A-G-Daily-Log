...
async function checkStockReportMovements() {
  const { data: items } = await supabase.from('items').select('id, name, code, dealer_price, srp, batch_note').order('created_at')
  const { data: movements } = await supabase
    .from('stock_movements')
    .select('item_id, direction, quantity, note')
    .not('note', 'like', 'Sales Report import:%')

  console.log(`Checking ${items?.length} items against ${movements?.length} Stock Report movements...`)
