    ...
    try {
      ...
      return await withSupabaseRetry(async (sb) => {
        const { data: items, error: itemsErr } = await sb
          .from('items')
          .select(`id, name, code, srp, categories ( name )`)
        ...
        for (const item of items) {
          ...
          if (item.itemCode && item.itemCode.trim()) {
            const { data: byCode } = await sb
              .from('items').select('id').eq('code', item.itemCode.trim()).limit(1)
            if (byCode && byCode.length > 0) existingId = byCode[0].id
          }
          if (!existingId && item.description && item.description.trim()) {
            const { data: byName } = await sb
              .from('items').select('id').ilike('name', item.description.trim()).limit(1)
  ...
  ipcMain.handle('inventory:addProduct', async (_event, payload: AddProductPayload): Promise<IpcResult<void>> => {
    try {
      return await withSupabaseRetry(async (sb) => {
        ...
        const { data: cat } = await sb.from('categories').select('id').ilike('name', catName).limit(1)
        if (cat && cat.length > 0) {
          categoryId = cat[0].id
        } else {
          const { data: newCat } = await sb.from('categories').insert({ name: catName }).select('id').single()
