      ...
      return await withSupabaseRetry(async (sb) => {
        ...
        const { data, error } = await sb
          ...
          .select(`
            ...
            items ( id, name, code, srp, categories ( name ) ),
            stock_movements ( buyer_id, buyers ( id, name ) )
          `)
          .gte('date', startDate)
          .lte('date', endDate)
        ...
        const sales: ItemSale[] = (data || []).map((row: any, idx: number) => {
          ...
          return {
            ...
            price, qty, salesAmount, discount, salesTotal,
            remarks: row.remarks || '',
            date: row.date,
            rowNum: idx + 8,
    ...
    try {
      return await withSupabaseRetry(async (sb) => {
        ...
        if (typeof rowNumOrId === 'string' && isNaN(Number(rowNumOrId))) {
          ...
          const { data } = await sb.from('item_sales').select('id, stock_movement_id')
          ...
          if (data && data[index]) { saleId = data[index].id; movementId = data[index].stock_movement_id }
        }
        ...
        if (saleId) {
          ...
          if (movementId) await sb.from('stock_movements').delete().eq('id', movementId)
        }
    ...
    try {
      return await withSupabaseRetry(async (sb) => {
        ...
        if (saleId) {
          ...
          await sb.from('item_sales').update({
            item_id: itemId,
            quantity: sale.qty || 0,
          ...
          if (movementId) {
            await sb.from('stock_movements').update({
              item_id: itemId,
              quantity: sale.qty || 0,
            ...
            const { data: newMov } = await sb.from('stock_movements').insert({
              item_id: itemId,
              direction: 'out',
              quantity: sale.qty || 0,
              buyer_id: resolvedBuyerId,
